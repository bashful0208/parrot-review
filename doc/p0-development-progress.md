...
>
> 当前工程基础大多已完成；当前代码仍保留 `SUPABASE_*` 历史命名，但这只是命名现状，不代表目标架构。当前数据库底座已切到自建 `PostgreSQL`，Auth / Realtime / Secrets / Storage 仍是能力待定与接口预留项。
...
- [ ] 明确稳定的用户主体标识契约；业务表中的 `user_id` / `owner_user_id` / `generated_by_user_id` 等字段统一引用该主体
- [ ] 明确 session 续期策略
- [ ] 明确服务端读取当前用户身份的方式
